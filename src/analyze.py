...
def allocate(ia,ib,requests,priority):
    budget=P['psu']['operational_cap_W'];fixed=47.84;used=fixed+24*(max(0,ia)+max(0,ib));on=[False]*4
    if used>budget:return on,0
    for i in priority:
        w=[100,100,100,60][i]
        if requests[i] and used+w<=budget:on[i]=True;used+=w
    return on,used
...
def main():
    ...
    budget={'psu_nameplate_W':800,'psu_current_A':33,'psu_current_derived_ceiling_W':792,'M1_rated_electrical_W':24*8.2,'M2_rated_electrical_W':24*1.8,'heater_installed_W':360,'fans_W':2*7.92,'controls_pull_spool_allowance_W':32,'nominal_all_on_W':24*(8.2+1.8)+360+47.84,'hardware_limit_proposal_A':[12,3],'bounded_all_on_at24V_W':24*(12+3)+360+47.84,'operational_cap_W':P['psu']['operational_cap_W'],'operational_cap_semantics':'HARD_REJECT_NEXT_DEMAND','actual_current_limit_and_transient_testing':'NOT_RUN'}
      ...
      for ib in np.linspace(0,3,13):
       for mask in itertools.product([False,True],repeat=4):
        for shift in range(4):
         ...
         assert used<=P['psu']['operational_cap_W'];maxallowed=max(maxallowed,used)
