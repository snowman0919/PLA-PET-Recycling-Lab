...
Jam 입력은 250 ms 창의 current RMS·peak·양의 미분, tach speed ratio와 vibration peak를 정규화한 load score다. Score와 speed-drop이 동시에 250 ms 지속되면 feed를 제한하고, 추가 500 ms 뒤 forward drive를 끈다. 300 ms 정지 확인 후 800 ms reverse, 1,000 ms retry를 최대 3회 허용한다. Persistent jam은 loop quantization을 포함해 7.372 s 이내 `FAULT_JAM`으로 latch된다. 계수와 threshold는 provisional이라 실제 inertia·파편 rebound·coupon dyno 검증 전 qualification lock을 열지 않는다.
