...
현재 STEP `e6793f06…`/동일 STEP 원본 USD `41ef9593…`의 19,200-step,
8-cycle 연결 실행에서 S2 스크린 bounding-box 근접은 5/200이며
...
- **능동 슈트**: 과거 수동 계단, 패들/스크레이퍼와 얕은 오거는
  ...
  BRep 비간섭이다. S1 팬 가이드를 y223..241의 주 오거 벽까지
  수렴시켜 국부 S1 픽업은 5/22에서 12/22로 증가했으나 아직
  비구동 정체·유실이 남았다. USDA 충돌 근사값은
  `UsdPhysics.MeshCollisionAPI`로 명시했다. 이 콜라이더 조건에서
  별도 슈트 주입의 x335 통과는 22/22, cap bore의 y255 +Y
  횡단은 12/22(횡단 이후 유실 5/22)다. 연결 시험의 스크린
  bounding-box 근접은 5/200뿐이다. 최종 판정은 `path_check.json`,
  `flow_localize/results.json`, `run_vp1_final18/results.json`의
  동일 STEP/USD 해시와 함께 읽는다.
- **풀러/와인더**: 1.75 mm 필라멘트용 스프링 니프, 양단 베어링,
  모터-드럼 토크 결합과 트래버스를 실제 조립 형상으로 포함한다.
- **전장**: PSU 24V 33A, 명판 800W. 500W는 경성 운전 예산,
  792W는 별도 PSU 전류 정격 유도 하드웨어 최대값이며 운전 허용치가 아니다.
  단계 제어 정상 피크는 416W(모터/팬은 미정격 모델 추정)다.
- **펌웨어**: host self-test 20건의 전력 할당 로직은 우선순위에 따라
  500W를 넘길 다음 수요를 거부하며 밴드 히터는 한 번에 하나만 허용한다.
