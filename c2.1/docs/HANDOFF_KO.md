...
현재 STEP/동일 해시 USD의 19,200-step, 8-cycle 연결 실행에서 S2 스크린
근접은 0/200이며 제품 소재 경로는 **HOLD**다. 구매·가공·통전, 물리
운전, main 병합은 명시적 승인 전 **HOLD**다.
...
- **능동 슈트**: 과거 수동 계단, 패들/스크레이퍼와 얕은 오거는
  실패 이력이다. 현재 4회전 오거와 +Y 횡이송 스크루/쉘은 BRep
  비간섭이지만 국부 시험에서 오거 x335 통과 21/22, 실제 S2 입구
  횡단 0/22, x335 이후 world 유실 13/22다. S1 픽업도 5/22다.
  최종 판정은 `path_check.json`, `flow_localize/results.json`,
  연결 `run_vp1_final16/results.json`의 해시와 함께 읽는다.
