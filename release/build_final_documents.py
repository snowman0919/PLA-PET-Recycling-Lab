...
ASSEMBLY_STEPS = [
    ...
    (2, "FR profiles ×40; FR-TIE-01 ×1; bracket allocation by member", "square; long steel tape/rule; 3/5 mm hex", "M5x12/washer/T-nut joint kits ×56", "M5 5 N·m", "470×700 base square", "base X470±0.8 mm; Y700±0.8 mm; rail squareness≤0.50/700 mm; all numeric limits include U95", "FR-001", "member-by-member joint/received T-nut engagement check; FR-TIE four M5x10 joints; independent X/Y and two-diagonal measurements with evidence hash; rocking check", "|diagonal A-B|+U95_A+U95_B≤1.0 mm; no rocking", "table anchors"),
...
def assembly_rows() -> list[dict[str, str]]:
    ...
    supplements = {1: "BOM/revision traveler ×1", 2: "FR profiles ×40; FR-TIE-01 ×1; bracket allocation by member",
    ...
    rows[2]["orientation"] += "; final GGM frame per exports/final/frame_v08/frame_members.csv; FR-TIE-01 U40x20x3 R3 L470 at X0/Y265/Z20; four M5x10+OD10 washer+matched T-nut; no old bottom L430 or dangling L59"
    rows[2]["drawing"] += "; exports/final/frame_v08/FRAME_CUT_AND_TIE_KO.pdf"
    rows[2]["clearance_tolerance"] += "; per-bar nesting consumes nominal+0.5 mm per part+kerf; received T-nut torque/engagement is a physical HOLD"
