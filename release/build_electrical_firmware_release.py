...
BASE_SOURCE = ROOT / "firmware/arduino_mega"
...
def power_wires() -> list[dict[str, str]]:
    ...
    wires = [
        ...
        row("24-MAIN+", "PSU +24 V", "F-MAIN", "24 VDC", "30 A F-MAIN protected machine envelope; PSU 33.3 A available", ">=6 mm2 Cu 90 C", "red", "locking DC terminal >=30 A 60 VDC", "TB24+", "F-MAIN 30 A DC", "high-current duct", "paired 0 V", "both ends clamped"),
        row("24-MAIN-", "PSU 0 V", "0 V star", "0 VDC", "30 A F-MAIN protected machine envelope; PSU 33.3 A available", ">=6 mm2 Cu 90 C", "black", "locking DC terminal >=30 A 60 VDC", "TB24-", "F-MAIN upstream", "high-current duct", "functional 0 V; not PE", "both ends clamped"),
    ]
    branches = (
...
OUTPUTS = {"SHREDDER_LPWM_PIN", "SHREDDER_ENABLE_PIN", "SCREW_DIR_PIN",
           ...
           "FAN_TACH_MUX_SELECT_PIN", "FEEDER_DIR_PIN", "FEEDER_ENABLE_PIN", "FEEDER_STEP_PIN", "SHREDDER_PWM_PIN", "SCREW_PWM_PIN",
           "PULLER_PWM_PIN", "SPOOLER_PWM_PIN", "COOLING_PWM_PIN",
...
def build_variant() -> dict:
    ...
    runpy.run_path(str(VARIANT_BUILDER), run_name="__main__")
    if not VARIANT_MANIFEST.is_file() or not SOURCE.is_dir():
        raise SystemExit("GGM variant generation failed")
    manifest = json.loads(VARIANT_MANIFEST.read_text(encoding="utf-8"))
    if manifest.get("profile") != "GGM-DRIVE-v0.8-r1" or manifest.get("hardware_enabled") is not False:
        raise SystemExit("unexpected GGM variant manifest")
    if any(sha(ROOT/p) != h for p,h in manifest.get("source_sha256",{}).items()):
        raise SystemExit("GGM variant base-source binding stale")
    return manifest


def tracked_sources() -> list[Path]:
    files = sorted(p for p in SOURCE.rglob("*") if p.is_file() and p.suffix.lower() in {".ino", ".h", ".cpp"})
    if not any(path.name == "arduino_mega.ino" for path in files):
        raise SystemExit("GGM firmware source incomplete")
    return files


def compile_hex(cli: str, sketch: Path, output: Path) -> tuple[Path, str]:
    result = subprocess.run([cli, "compile", "--fqbn", FQBN, "--clean", "--build-path", str(output / "build"), "--output-dir", str(output), str(sketch)], cwd=ROOT, text=True, capture_output=True)
...
""")
    ...
    magic_match = re.search(r"CALIBRATION_RECORD_MAGIC\s*=\s*(0x[0-9A-Fa-f]+)UL", cal_text)
    if not magic_match: raise SystemExit("GGM calibration magic missing")
    magic = magic_match.group(1)
    write(FW/"EEPROM_schema.md", f"""# EEPROM schema v4 — GGM drive variant

Magic `{magic}`; version `4`; {len(cal)} domains: {', '.join(f'`{x}`' for x in cal)}.
...
def firmware_release(generator_commit: str, variant_manifest: dict) -> tuple[str,int,int]:
    cli = tool("arduino-cli")
    base_source_commit = subprocess.check_output(["git","log","-1","--format=%H","--","firmware/arduino_mega"], cwd=ROOT, text=True).strip()
    variant_builder_commit = subprocess.check_output(["git","log","-1","--format=%H","--","firmware/ggm_drive_v08"], cwd=ROOT, text=True).strip()
    ...
    compiler = subprocess.check_output([str(Path(compiler_path)/"bin/avr-g++"),"--version"], text=True).splitlines()[0]
    manifest = {"schema_version":1,"status":"PASS","release_revision":REV,
      "source_kind":"GGM_BTS7960_VARIANT","generation_base_git_sha":generator_commit,
      "base_firmware_last_commit":base_source_commit,"variant_builder_last_commit":variant_builder_commit,
      "variant_source_dir":str(SOURCE.relative_to(ROOT)),"variant_manifest":str(VARIANT_MANIFEST.relative_to(ROOT)),
      "variant_manifest_sha256":sha(VARIANT_MANIFEST),
      "variant_builder_sha256":sha(VARIANT_BUILDER),
      "variant_base_source_sha256":variant_manifest.get("source_sha256",{}),
      ...
      "libraries":[],"exact_build_command":"arduino-cli compile --fqbn arduino:avr:mega --clean --build-path <clean-dir>/build --output-dir <clean-dir> source/arduino_mega",
    ...
    firmware_docs(pins(), generator_commit, f"base:{base_source_commit}; variant-builder:{variant_builder_commit}")
...
def main() -> None:
    ...
    variant_manifest = build_variant()
    wire_count, pin_count = electrical_release(commit)
    digest, flash, sram = firmware_release(commit, variant_manifest)
