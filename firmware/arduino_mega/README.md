...
`shredder_control`은 MY1016Z direct drive를 대상으로 PLA/PET 32/24 rpm, 16/18 A 지속 과부하, 명령속도 대비 35% 이상 RPM drop, profile별 reverse duration과 최대 3회 retry를 처리한다. 세 번째 reverse 종료 뒤 fault를 latch하며, heater 또는 screw enable과 shredder enable은 상호 배제한다. 이 host core는 BTS7960 board의 실제 PWM pin driver나 current calibration을 대신하지 않는다.
