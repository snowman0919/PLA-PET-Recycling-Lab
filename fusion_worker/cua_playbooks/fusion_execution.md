...
1. `environment_check.ps1` PASS와 exact checkout을 확인한다.
...
6. result CSV를 채우고 run manifest를 `PASS` 또는 `FAIL`로 완료한다. solve 미실행은 `PENDING`, login/license/cloud 승인은 `BLOCKED_EXTERNAL`이다.
