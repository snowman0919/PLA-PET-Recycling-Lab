...
from run_calculix_v08 import mesh_step,read_gmsh_inp,nset,solve
RAW=H/'raw/plate';RAW.mkdir(exist_ok=True)
...
for label,source in [('before',old),('r2',new)]:
  for size in (1.8,1.2,.8):
    work=RAW/(label+'_'+str(size));work.mkdir(exist_ok=True)
    mesh_file=work/'gmsh.inp' if '--reuse' in sys.argv and (work/'gmsh.inp').exists() else mesh_step(source,work,size)
    nodes,elements=read_gmsh_inp(mesh_file)
    counts=Counter()
    for e in elements:
        v=list(map(int,e.split(',')))[1:]
        for a,b,c in ((0,1,2),(0,1,3),(0,2,3),(1,2,3)):counts[tuple(sorted((v[a],v[b],v[c])))]+=1
    w=defaultdict(float)
    ...
    deck += ['*ELEMENT,TYPE=C3D4,ELSET=ALL',*elements,'*SOLID SECTION,ELSET=ALL,MATERIAL=STEEL',
...
result={'physical_validation':'NOT_RUN','machine_release':'HOLD','scope':'Local cold elastic plate comparison only; no bolt preload, hot joint, frame or contact validation',
...
(H/'retention_strength.json').write_text(json.dumps(result,indent=2)+'\n')
