...
from cad.freecad.compact.generate import normalize_step,_projection_polylines
...
def export(name,items):
    ...
    for row in items:
        obj=doc.addObject('PartDesign::Feature',row['name']);obj.Shape=row['shape'];features.append(obj)
    doc.recompute();path=OUT/(name+'.step');Part.export(features,str(path));normalize_step(path)
    native=OUT/(name+'.FCStd');doc.saveAs(str(native));App.closeDocument(doc.Name)
...
def main():
    paths=list(Path(__file__).parent.glob('*.py'))+[ROOT/'control/ggm_drive_contract.json']
    ...
    result={'status':'NEW_DRIVE_CLEARANCE_PASS' if not review['new_interference'] else 'NEW_INTERFERENCE_FOUND',
            'machine_release':'HOLD','physical_validation':'NOT_RUN','review':review,'exports':rows,'source_sha256':bindings,
