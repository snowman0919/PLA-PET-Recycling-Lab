...
def write_machine_fabrication_package():
    ...
    with (base / "frame_cut_list_baseline_reference.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f, lineterminator="\n")
    ...
    (base / "README_ko.md").write_text(
        ...
        "최종 GGM 프로파일은 release/build_frame_release.py가 생성하는 frame_cut_list.csv를 따른다. frame_cut_list_baseline_reference.csv는 구형 기본 배치 참조이며 절단에 사용하지 않는다. "
...
def write_thermal_package():
    ...
    specs=[
        dict(id="TH-BH-01",name="Custom barrel mica band heater Z2/Z3",shape=mica_band_heater_shape(),qty=2,material="mica/NiCr/stainless sheath",process="custom heater RFQ",critical="24 VDC 100 W each; free-state ID34.10–34.20; usable split-closure travel >=1.00; width45 ±0.5; radial build2 nominal; cold resistance 5.76 Ω ±10%; 300 mm fiberglass leads; PE-bonded sheath; PET service 300 C design; no stock Ø35 substitution"),
        dict(id="TH-BH-02",name="Custom barrel mica band heater Z1",shape=mica_band_heater_shape(width=40),qty=1,material="mica/NiCr/stainless sheath",process="custom heater RFQ",critical="24 VDC 100 W each; free-state ID34.10–34.20; usable split-closure travel >=1.00; width40 ±0.5; radial build2 nominal; cold resistance 5.76 Ω ±10%; 300 mm fiberglass leads; PE-bonded sheath; PET service 300 C design; no stock Ø35 substitution; reduced-width 100 W design requires supplier watt-density and lead-layout review; thermal response NOT_RUN"),
...
""",encoding="utf-8")
    with (base/"channel_schedule.csv").open("w",newline="",encoding="utf-8") as f:
        ...
        w.writerows([
            ("HZ1","TH-BH-02 zone1 W40",100,"4.17","T1 K-type","F-H1 branch fuse + K0 dual thermal-cutoff chain","MOSFET1 low-frequency","HOLD"),
