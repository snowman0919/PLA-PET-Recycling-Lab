...
58행 시스템 BOM 중 BUY 23행을 모두 `procurement_routes.csv`에 연결했고, 그중 12개 Part ID에 실제 공개 페이지 또는 Playwright 검색 증거를 기록했다. `PRIMARY_CANDIDATE`와 qualification/sizing 후보는 가격·배치 계산용 기준일 뿐 구매·안전 적합성 승인이 아니다.
...
- AliExpress 3개 검색 결과(3개 BOM evidence 행)는 Playwright Chromium으로 직접 읽었다. 배송·세금·seller·variant·정품 여부가 확정되지 않아 모두 `SAMPLE_ONLY`이고 planning primary로 선택하지 않았다.
