...
총 58개 line item, CRITICAL 43개다. 이 문서는 `build_design_boms.py`가 `bom.csv`에서 생성한 검사 요약이며 주문서가 아니다.
...
| Spooler | 1 |
| Stage 1 | 8 |
| Stage 2 granulator | 9 |
| User interface | 2 |

## 비용 상태

공개 primary 후보 2개 품목의 planning floor는 148,321 KRW로 200,000 KRW cap을 -51,679 KRW 초과한다. 나머지 부품·가공·미확정 배송·세금은 포함하지 않았다.
...
- `procurement_routes.csv`: 23개 BUY 행의 권장 공급처·대체 공급처·AliExpress 허용 경계를 기록한다.
