...
namespace {
constexpr const char *ACTUATION_REVISION = "ggm-drive-v08-r1";
...
class BoardActuators final : public ActuatorBackend {
 public:
  ...
  void apply(const ActuatorCommands &c) {
    ...
    const bool sh_tach_valid=calibrationDomainReady(calibration_record,CAL_SHREDDER_TACH) && shredder_tach_sample.valid;
    ...
    const uint16_t requested_hz = c.feeder_enable && verified && !safe.fault && safe.screw>0 ? c.feeder_step_hz : 0;
    ...
    writeBts(Board::SCREW_PWM_PIN,Board::SCREW_DIR_PIN,Board::SCREW_ENABLE_PIN,safe.screw);
    ...
    const uint8_t mask=verified && !safe.fault && gi.safety_ok && !c.shredder_pwm ? GgmDriveGuard::heaterMask(request,c.shredder_pwm || c.screw_pwm,(millis()/250)%4):0;
 ...
 private:
  ...
  static void writeBts(uint8_t right_pwm,uint8_t left_pwm,uint8_t enable,int16_t value) {
...
void executeCommand(char *line, InputSnapshot input, uint32_t now_ms) {
  ...
  if (strcmp(verb, "MATERIAL") == 0) {
    ...
    if (accepted) purge_feed_approved = false;
  } else if (strcmp(verb, "GGM_REPORT") == 0) {
