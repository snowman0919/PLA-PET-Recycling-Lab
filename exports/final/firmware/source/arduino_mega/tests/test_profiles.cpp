...
int main() {
  ...
  assert(c.requestState(MachineState::REQUALIFYING, safe));
  ...
  assert(c.materialSession() == MaterialSession::PURGE_PREHEAT_REQUIRED);
  assert(!c.requestState(MachineState::PREHEATING, safe));
  assert(c.requestPurgePreheat(safe));
  assert(c.markPurgeReady(safe));
  assert(c.startPurge(true, safe));
  assert(!c.completePurgeRun(false));
  assert(c.completePurgeRun(true));
  assert(c.state() == MachineState::COOLDOWN && c.permissions().cooling);
  assert(c.requestState(MachineState::IDLE, safe));
