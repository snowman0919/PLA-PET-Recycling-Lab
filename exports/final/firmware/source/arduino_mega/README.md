...
Shredder/screw/puller/spooler drive와 tach, traverse, gauge, current, fan 1/2 tach, dancer, cooling-current calibration readiness는 서로 독립이다. EEPROM record v4는 domain별 source/verified/range/units/revision/CRC와 전체 CRC가 일치하지 않으면 해당 production 출력을 승인하지 않고 v3 이하를 무효화한다. 재질 선택은 어떤 calibration도 암시하지 않는다. Shredding start와 extrusion arm은 subsystem 승인이 끝난 뒤에만 process phase를 commit한다. Fault clear는 모든 subsystem의 `canClearFaults` preflight가 통과한 뒤 한 번에 commit하며 clear 자체가 actuator를 재시작하지 않는다.

재질 변경은 `PURGE_PREHEAT_REQUIRED -> PURGE_READY_CONFIRM_REQUIRED -> PURGE_RUNNING -> SCREEN_CLEAN_REQUIRED -> HOPPER_CLEAN_REQUIRED -> TEMPERATURE_TRANSITION_REQUIRED -> FINAL_CONFIRM_REQUIRED` 순서를 강제한다. Purge 전에 작업자가 물리 `START`로 purge feed를 승인하고 별도 `CONFIRM`으로 기존 waste tray/manual waste path를 확인해야 한다. 80 g/120 g은 명목 engineering estimate일 뿐 측정 질량이 아니며, 완료 증거는 최소 120초, verified A13 timestamp tach로 적분한 실제 screw 32회전, 온도 band 유지, drive/heater 정상 및 육안 확인이다. tach timeout이나 command-motion mismatch에서는 회전 증거를 무효화한다. 중단·E-stop·fault clear 뒤에는 pending material을 활성화하지 않고 `PURGE_PREHEAT_REQUIRED`로 돌아간다.
...
Mega adapter는 A13 timestamp pulse를 hybrid tach estimator에 넣고 actual screw RPM을 적분한 경우에만 purge revolution credit를 준다. Tach calibration/신호가 없거나 timeout, command-motion mismatch, driver fault가 있으면 `purge_screw_revolutions_measured=false`가 되어 완료할 수 없다. Donor tach target의 12 PPR와 전기 level은 commissioning에서 라벨/실측으로 확인해야 하며 reference default는 verified calibration이 아니다.

Shredder, screw, puller, spooler는 모두 channel별 tach와 bounded PI를 사용한다. 저속은 reciprocal period, 고속은 window count를 사용하며 fixed 20 ms count/divisor 경로는 없다. Traverse는 calibration만으로 움직이지 않고 Serial `HOME_TRAVERSE` 또는 동등한 explicit operator intent 뒤 left-limit 탐색과 2 mm backoff를 완료해야 winding이 허용된다.
...
`src/generated_profiles.h`는 `generate_config.py`가 `cad/parameters/baseline.json`, `control/process_contract.json`, `control/fault_response_contract.json`에서 생성한다. Tach 상수는 `control/tach_contract.json`에서 `control/generate_tach_contract.py`가 생성한다. Modelica `GeneratedControl.mo`의 유일 writer는 `control/generate_contract_artifacts.py`다. Screw setpoint는 PLA 16 rpm/PET 18 rpm, shredder는 PLA 32 rpm/PET 24 rpm이며, 외부 pre-dry는 현재 둘 다 `UNQUALIFIED_EXTERNAL_PROCESS`다.
