...
- **소재 이송은 HOLD**: 현재 STEP `b4acee8c…`/USD `e484708c…`에서
  8-cycle 국부 시험은 호퍼 입구 22/22, S1 배출 판정면 22/22지만
  S1에서 오거 픽업은 5/22다. S1과 분리해 슈트에 주입한 파편은
  x335 22/22, S2 cap bore의 y255 +Y 횡단 18/22다. 횡단 이후
  world 유실 6/22, 횡단 전 유실 2/22로 전체 이송 합격이 아니다.
  S2 국부 주입 22/22도 입구부터 별도 주입한 결과다.
  19,200 step(96 s, 8 입력 회전) **연결 실행**의 S2 스크린
  *bounding-box 근접*은 **1/200**(Ø1.5 mm 1개), Ø3 mm 0/100이다.
  `product_flow_verified=false`, 판정 `PARTIAL_PROBE_REACH`.
  구동 추적 PASS와 무예상 접촉, 스크린 근접은 구멍 통과/완성 필라멘트의
  증거가 아니다. `path_check.json`도 플라이트-입구 수동 간극
  3.28 mm 때문에 `all_material_path_clear=false`다.
...
- S1 배출 후 주 오거 픽업은 5/22에 그친다. 별도 슈트 주입에서는
  S2 cap bore로 18/22가 전달됐지만 일부가 월드로 유실된다.
  연결 호퍼 주입의 스크린 근접은 1/200뿐이며 스크린 구멍 통과
  및 압출 제품은 관측하지 않았다. S1 팬의 광폭 y162..325 배출을
  y223..241 오거 픽업대로 모으는 연속 능동 공급, S2 캡 이후
  포집/스크린 출구를 실제 CAD·충돌 형상과 연결 시험으로
  재설계·검증해야 한다. 국부 판정이나 스크린 근접을 제품 PASS로
  전환할 수 없다.
