...
- 문서 상태: Baseline v0.1
- 기준일: 2026-08-28
- 대상: PLA/PET 기반 모듈형 자동 폐플라스틱 필라멘트 재생기
...
| REQ-FUNC-002 | TPU와 불확실한 재질은 PLA/PET로 강제 분류하지 않고 UNKNOWN/REJECT로 보낸다. | 낮은 confidence 샘플의 reject 기록 | T,D |
| REQ-FUNC-003 | 재질 판별은 카메라 특징과 파쇄 load/current 또는 속도·진동 특징을 융합한다. | feature log와 fusion decision trace 존재 | I,T |
| REQ-FUNC-004 | 색상은 6개 고정 범주와 Reject로 분류한다. | bin mapping과 calibration 결과 일치 | I,T |
| REQ-FUNC-005 | 파쇄는 대형, 중간, granulator의 물리적 3단으로 구성한다. | 3개 cutter stage와 단계별 입도 샘플 | I,T |
| REQ-FUNC-006 | 이송·선별은 Oversize, Acceptable flake, Dust/fines의 세 흐름을 제공한다. | screen 분리와 각 배출 경로 시연 | I,D |
...
| REQ-FUNC-013 | Arduino Mega는 안전·실시간 제어, Raspberry Pi 4는 영상·분류·로그를 담당한다. | Pi disconnect 때 Mega 안전정지 | I,T |
| REQ-FUNC-014 | 장치는 batch와 생산 이력, 재활용 세대, 직경 통계를 저장한다. | 평균·표준편차·최소·최대·불량구간 export | I,T |
| REQ-PERF-001 | 안정 연속 처리량 목표는 200 g/h 이상이며 명목값과 분리해 보고한다. | 30분 이상 질량 수지 시험; `>=200 g/h` | T |
...
| REQ-PERF-004 | hopper 유효 적재량은 약 1.0~1.5 kg이며 운전 중 안전한 보충이 가능해야 한다. | 질량 시험과 anti-reach 접근성 검사 | I,T |

## 기계·열·전기 인터페이스

| ID | 요구사항 | 합격 기준 | 검증 |
|---|---|---|---|
| REQ-MECH-001 | 최대 평면 외형 약 200 x 200 mm의 입력과 500 mL급 PET 병을 받을 수 있다. | gauge fixture를 통과시키되 허용 두께는 시험 후 명시 | I,T |
...
| REQ-THERM-001 | barrel은 기본 3 zone과 die heating을 검토하고 재질별 profile을 독립 제어한다. | zone별 센서·heater·recipe trace | I,T |
...
| REQ-SAFE-002 | hopper lid와 service cover는 직접 cutter drive enable chain을 차단한다. | switch 단선·cover open 시험 | I,T |
...
| REQ-SAFE-006 | Pi 장애, 통신 단절, sensor open/short, watchdog timeout에서 Mega가 안전 상태로 간다. | fault injection matrix 전 항목 통과 | T |
