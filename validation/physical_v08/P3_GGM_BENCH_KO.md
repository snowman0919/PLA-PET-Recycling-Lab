...
P3는 cutter, screw, heater를 장착하지 않은 상태에서 두 GGM 구동축의 **실제 속도·전류·토크 대응과 기계식 보호핀**을 교정하는 단계다. 실행에는 P0 PASS, 동일 GGM receipt packet에 결박된 `P2_STAGE_RELEASE_VALIDATED`, GGM 수령·mount compatibility PASS, 배선 점검, 독립 기준계측 준비와 별도 사용자 통전 승인이 필요하다.
...
Preflight의 `p2_applicable_cold_fit` evidence는 P2 analyzer JSON이 아니라 `validate_p2_stage_release.py`로 현재 재검증 가능한 P2 stage-release JSON이어야 하며 P3가 사용하는 GGM receipt packet SHA와 동일해야 한다. 이어서 rigid metal load path, 12 mm/2x6201 test arbor, coupling/brake guard, cutter·screw 부재, heater branch 격리, 한 축씩만 통전하는 정책, BTS7960 두 채널, A0 shredder/A9 extruder motor-lead current wiring, F-SH 20 A/F-SCREW 10 A branch protection, de-energized hard-cut continuity, E-stop/positive-action interlock, 독립 current/RPM/torque 기준계측기와 P3 fixture 한정 승인 기록을 함께 확인한다. 수령 packet은 현 D02/D03와 `AS_DRAWN_COMPATIBLE_NOT_AUTHORIZED`여야 한다.
