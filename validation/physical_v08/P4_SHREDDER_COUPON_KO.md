...
P4의 목적은 full 12-disc stack 발주 전에 CUT-01 형상, screen, phase pair, GGM 구동/복구가 실제 PLA/PET에서 유효한지 확인하는 것이다. P3 GGM bench PASS가 선행조건이다.
...
기존 `exports/jigs/gate1/`의 preflight, gate1_results, jam_recovery, chip_size CSV를 사용한다. 결과 파일은 template을 덮어쓰지 말고 별도 run directory에 복사해 작성한다. `analyze_p4_records.py`가 네 CSV와 P3 PASS evidence를 함께 검사한다.
