...
3. `machine_fabrication_parts()` - 28 edges
4. `evaluate()` - 12 edges
5. `evaluate()` - 12 edges
6. `validate()` - 12 edges
7. `joined()` - 12 edges
8. `evaluate()` - 11 edges
9. `evaluate()` - 11 edges
10. `integrated_objects()` - 11 edges
...
Cohesion: 0.67
Nodes (3): collar_shape(), Purchased collar reference geometry, not a machining master or load rating., traverse_collar_rows()
