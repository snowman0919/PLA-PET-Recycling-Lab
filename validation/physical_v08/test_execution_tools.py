...
class ExecutionToolsTest(unittest.TestCase):
    ...
    def test_profile_nesting_synthetic(self):
        ...
        ok,plan=nest.solve(req['2020'],[('SYN-2020',18000.0)],2.0)
        self.assertTrue(ok); self.assertEqual(sum(len(x['cuts']) for x in plan),36)
        ...
        with tempfile.TemporaryDirectory(dir=HERE) as td:
            ...
            fields=['record_id','profile_type','source_asset','usable_length_mm','u95_length_mm','straightness_note','damage_note','instrument_id','instrument_calibration_ref','measured_at','operator','reviewer','evidence_path','sha256','status','notes','condition_check','inspection_power_state']
            with csv_path.open('w',newline='') as f:
                w=csv.DictWriter(f,fieldnames=fields); w.writeheader()
                common={'source_asset':'LAB','u95_length_mm':'1.0','straightness_note':'OK','damage_note':'NONE','instrument_id':'MEAS-16','instrument_calibration_ref':'CHECK-1M','measured_at':'2026-09-10T14:30+09:00','operator':'TEST','reviewer':'REVIEW','evidence_path':rel,'sha256':digest,'status':'USABLE','notes':'','condition_check':'USABLE_REVIEWED','inspection_power_state':'NONE'}
                w.writerow({'record_id':'S2020','profile_type':'2020','usable_length_mm':'18000',**common})
                w.writerow({'record_id':'S2040','profile_type':'2040','usable_length_mm':'3000',**common})
            stock=nest.measured(csv_path)
            self.assertEqual(stock['2020'][0][1],17999.0)
