...
def main() -> None:
    ...
    assert release["shredding"] == "TWO_STAGE_STAGE1_PLUS_SCREEN_GRANULATOR"
    assert release["classification"] == "MANUAL_SINGLE_MATERIAL_SINGLE_COLOR_BATCH"

    a = report["tower_a"]
    b = report["tower_b"]
    assert a["rack_envelope_mm"] == {"width": 500.0, "depth": 500.0, "height": 1100.0}
    assert a["maximum_input_lip_height_from_floor_mm"] <= 1050.0
    assert b["cooling_length_mm"] == 440.0
    assert b["die_to_gauge_center_mm"] == 470.0
    assert b["rack_envelope_mm"] == {"width": 850.0, "depth": 500.0, "height": 1000.0}
    assert b["straight_service_rail_extension_from_die_mm"] == 700.0

    batch = report["batch_interface"]
    assert batch["nominal_flake_capacity_kg"] == 0.5
    assert batch["maximum_handled_mass_kg"] == 0.7
    ...
    assert b_stability["unanchored_tip_acceleration_g"] >= 0.4
