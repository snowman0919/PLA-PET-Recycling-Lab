...
74 scenario PASS는 선택한 reduced-order 방정식과 가정 안의 가상 검증이다. 실제 PLA/PET 절단토크, chip size, melt flow와 filament tolerance를 측정했다는 뜻이 아니다. Optional Gate-1 미수행은 `main`을 차단하지 않지만 full cutter/screw-barrel 발주와 commissioning은 사용자 승인 전 잠겨 있다.
