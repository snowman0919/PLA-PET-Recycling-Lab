...
`nix develop --command python3 validation/run_all.py`가 다음 24개 gate를 순서대로 실행했다.
...
13. 288개 release artifact의 size/SHA-256 manifest와 112개 7-view PNG
14. A4 한국어 PDF 20쪽/14쪽의 header/EOF/page object 구조

최종 marker는 `ALL_AUTOMATED_VALIDATIONS_OK (24 gates)`였다. Manifest는 `artifacts/manifest.json`이며 모든 288개 항목의 현재 bytes와 SHA-256가 재검사됐다.
...
`v0.1.0-preflight` 기준 별도 임시 clean clone에서 generator→render→PDF→22-gate 전체 재실행이 PASS했다. 이후 firmware integration·요구사항 추적·RFQ package gate를 추가한 현재 작업본도 Nix 환경에서 FreeCAD 11개를 포함한 24개 gate 전체가 PASS했다. 현재 revision의 별도 clean-clone 재생성은 아직 OPEN이다. 최종 preflight clean clone에서는 STEP 변경 0건, STL/DXF/render/PDF/review JSON 변경 0건을 확인했다.
