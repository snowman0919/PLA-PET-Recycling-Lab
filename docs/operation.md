...
4. 완료는 최소 elapsed time, 최소 screw revolution evidence, stable temperature band, pressure/drive fault 없음, 작업자 시각 확인을 모두 요구한다. Revolution evidence는 A13 screw Hall channel의 verified timestamp pulse를 적분한 `ACTUAL_SCREW_TACH_MEASURED_REVOLUTIONS`만 인정한다. Tach timeout, command-motion mismatch 또는 driver fault이면 증거를 무효화하고 purge를 중단한다. 80 g/120 g 표기는 공학 추정치일 뿐 측정 purge mass가 아니다.
